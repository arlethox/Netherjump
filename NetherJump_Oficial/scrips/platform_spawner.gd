...
@export var min_horizontal_spacing: float = 80.0  # Distancia mínima horizontal entre plataformas
@export var spawn_area_width: float = 190.0

# SPAWNING
@export var spawn_interval: float = 0.0  # Intervalo de aparición en segundos
@export var spawn_chance: float = 0.5  # Probabilidad de que aparezca una moneda en una plataforma
@export var mobile_platform_chance: float = 0.4  # Probabilidad de crear una plataforma móvil adicional
...
func _process(delta):
	...
	# Eliminar plataformas fuera del rango visible
	var player = get_node(player_path)
	var viewport_bottom = player.position.y + 200  # Ajusta este valor según el tamaño de tu ventana
	for platform in platforms_queue:
		if is_instance_valid(platform) and platform.position.y > viewport_bottom:
			platform.queue_free()
			platforms_queue.erase(platform)
...
func _on_player_touched_platform(platform):
	...
	delete_timer.wait_time = 2.0  # Configurado a 3 segundos
...
func spawn_mobile_platform(spawn_position: Vector2):
	...
	platform.move_direction = Vector2(1, 0) if randf() < 0.5 else Vector2(0, -1)  # Movimiento horizontal o vertical
	platform.move_speed = 50 + randf() * 50  # Velocidad entre 50 y 100
	platform.move_range = 50 + randf() * 150  # Rango de movimiento entre 50 y 200
	
	# Añadir la plataforma móvil a la cola
	platforms_queue.append(platform)
	
	# Crear un temporizador para eliminar la plataforma después de un tiempo
	var delete_timer = Timer.new()
	delete_timer.wait_time = 4.0  # Tiempo en segundos antes de eliminar
	delete_timer.one_shot = true
	platform.add_child(delete_timer)
	delete_timer.connect("timeout", Callable(platform, "queue_free"))
	delete_timer.start()
